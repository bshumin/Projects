...
//   and signal, as weall as the necessary init and destroy functions
//
...
void resource_reclaim( resource_t *r ){
    ...
    pthread_cond_destroy( &r->lock_cond);
...
int resource_allocate( struct resource_type_tag *self, int tid ){
    ...
    while(self->available_count == 0){
        pthread_cond_wait(&self->lock_cond,&self->lock_mutex);
    }
    ...
    // pthread_cond_signal(&self->lock_cond);
...
void resource_release( struct resource_type_tag *self, int tid, int rid ){
    ...
    //   as appropriate; follow the patterns in the textbook
    pthread_mutex_lock(&self->lock_mutex);
    // while(pthread_cond_wait(&self->lock_cond,&self->lock_mutex));
